<#
.Synopsis
  This tool is for testing remote Artifactory repositories. The tool can be utilised via the PowerShell command line.
...
########## Remove the package from the Artifactory repo ##########

Invoke-RestMethod -Uri "https://$url/$grpName/" -Method DELETE -Credential $Auth -Headers @{ Authorization = "Basic" } -ErrorVariable RestErrorDel

if ($RestErrorDel)
{
    $HttpStatusCodeDel = $RestErrorDel.ErrorRecord.Exception.Response.StatusCode.value__
    $HttpStatusDescDel = $RestErrorDel.ErrorRecord.Exception.Response.StatusDescription
}
