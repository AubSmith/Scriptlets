...
# 
# COMMENT: 
# Uses Params from allow modification of script at runtime
# Uses funHelp function to display help
# Uses funLine function to underline output
# Restores all GPOs from the most recent backup
#
# =============================================================================
...
@"
 ...
 RestoreAllGPOs.ps1  -path "c:\gpoBackup"

 Restores all GPOs in the default domain from the c:\gpoRestore 
 directory. 

 RestoreAllGPOs.ps1  -path "c:\gpoBackup" -domain "nwtraders.com"

 Restores all GPOs in the nwtraders.com domain from the c:\gpoRestore 
 directory. 

 RestoreAllGPOs.ps1 -path "c:\gpoBackup" -domain "nwtraders.com -whatif
 
 Displays what if: Perform operation Restore all GPOs  in nwtraders.com
 domain from the c:\gpoBackup folder 

 RestoreAllGPOs.ps1 -help

 Prints the help topic for the script

 RestoreAllGPOs.ps1 -help -full

 Prints full help topic for the script

 RestoreAllGPOs.ps1 -help -examples

 Prints only the examples for the script

 RestoreAllGPOs.ps1 -examples
...
Function funWhatIf()
{
 "what if: Perform operation Restore all GPOs  in $domain domain
           from the $path folder"
 exit
} #end funWhatIf

Function funRestoreGPO()
{
 if( -not $(test-path $path)) { $(throw "$path can not be found") }

 $gpm = new-object -comobject GPMgmt.gpm
 $constants = $gpm.getConstants()

 $gpmDomain = $gpm.getDomain($domain,$null, $constants.UseAnyDC)
 $gpmBackupDir = $gpm.GetBackupDir($path)
 ...
 if($backupList.Count -eq 0)
  {
   Funline -strIn "No backups found at $path" -scolor red -ucolor white -char #
   exit
  } #end if
 ELSE
  {
   Funline("Found $($backupList.count) gpo's at $($gpmBackupDir.BackupDirectory)")
  } #end else
 ...
 foreach($backup in $backuplist)
  {
   $error.clear()
   $gpmBackup = $backup
   Funline("Processing backup GPO $($gpmBackup.gpoDisplayName)")
   If($($gpmBackup.gpoDomain) -ne $($gpmDomain.Domain))
     { 
      "$($gpmBackup.gpoDomain) not equal to $($gpmDomain.Domain)" #debug
      "skipping backup of gpo $($gpmbackup.gpoDisplayName) due to different domain name"
     }
   
   $gpmResult = $gpmDomain.RestoreGPO($gpmBackup,$constants.DoNotValidateDC)
   $gpmResult.overallstatus()
   if($error.count -ne 0)
    { 
     funline -strin "An error occurred Restoring $($GPMBackup.GPODisplayName)
               it was $($error.categoryInfo)" -scolor red
    } #end if
   else
    {
     $gpmRestore = $gpmResult.result
...
if($full)      { funhelp }
if($whatif)    { funWhatIf }
if($paramMissing) { "missing path" ; funhelp }
if($path) { funRestoreGPO }

# SIG # Begin signature block
# MIIkCgYJKoZIhvcNAQcCoIIj+zCCI/cCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQU+WyyBBVqKcVQYTjLW2U5Z/bv
# IMuggh7hMIIEEjCCAvqgAwIBAgIPAMEAizw8iBHRPvZj7N9AMA0GCSqGSIb3DQEB
...
# AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUqWBHdrOLfZ3KEyLt
# kpJaRnmkCaUwXgYKKwYBBAGCNwIBDDFQME6gJoAkAE0AaQBjAHIAbwBzAG8AZgB0
# ACAATABlAGEAcgBuAGkAbgBnoSSAImh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbS9s
# ZWFybmluZyAwDQYJKoZIhvcNAQEBBQAEggEABPI7+cPEGL7RpoBy6OCaGAKsEUvF
# kGfiZcJRQ2tH6ALuod3RxKMPnksOOSnLuRxKmpq3yXBNnW8Aexv/I//+evqj6mTA
# KwSWUzQuz2oSlJQDJaIFyZZ613OX6nRh9i++VJM+BFOP71shi/OH1QPRI7olBCoF
# S7Kl8FmokogH5D0p2V15S1TO7jdNYp5w989rd4XE+ZrBh/ndir4mnQ5x7BFRwjrI
# bLEFy2ThFSlXyx1jVy7xlINLwsbXo1cpIsETQGuLOSqDjwt03m9J86BbqmbvcwFz
# CezO+JvY9GEmOQ3/hT41+XYEkHqzwZDOpypjPEDqBIdyrK+VgBsxhuz8QaGCAh8w
...
# AQkEMRYEFDTqw1kGbmh8oMFcbo2Nl2sVV2sJMA0GCSqGSIb3DQEBBQUABIIBAFfK
# K07dHkUJZZtWFg/i7o7KhD8H0d7O7AiKp/TNCdCDu4W5Y7MouBMuGj0XFNRNtsBf
# 9Y9uyvnvynswupNx1PjIOKrn9jvwfeJp8EhNwMnl5I9/D8DfdSmMWK/KV0Y1mX25
# xL1KfFJUSBUEknF5EWwfiR2GZHk+NlrnDs7wLBzrhDmkeE94hNKS6IikMojIHKYo
# LtBEvKJMKkBYF44IyaEQ4/SUWfdoy4tBOLQnA2hY85FzwOvW0EJsK1NfBIbUj4BJ
# qVvwUTjD8jLnUHAetL/wetKPkcBfbNsvIEtOmbrdhYqFxUHevy03Al1aZKTq2Dl4
# fk3Z9tHEJa7dLwGotLA=
