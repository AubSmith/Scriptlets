...
@"
 ...
 BackupAllGPOs.ps1  -path "c:\gpoBackup" -comment "gpo backup"

 Backs up all GPOs in the default domain to the c:\gpobackup 
 directory. The backup will contain the benign "gpo backup" 
 comment. 

 BackupAllGPOs.ps1 -path "c:\gpoBackup" -comment "gpo backup" 
 -domain "nwtraders.com -whatif
 
 Displays what if: Perform operation backup all GPOs  in nwtraders.com
 domain to the c:\gpoBackup folder with a comment of gpo backup

 BackupAllGPOs.ps1 -help

 Prints the help topic for the script

 BackupAllGPOs.ps1 -help -full

 Prints full help topic for the script

 BackupAllGPOs.ps1 -help -examples

 Prints only the examples for the script

 BackupAllGPOs.ps1 -examples
...
Function funWhatIf()
{
 "what if: Perform operation backup all GPOs  in $domain domain
           to the $path folder with a comment of $comment"
 exit
} #end funWhatIf

Function funBackupGPO()
{
 ...
 Funline("Found $($gpoList.count) gpo's in $($gpmDomain.domain)")

 foreach($gpmGPO in $gpolist)
  {
   $error.clear()
   $gpmResult = $gpmGPO.backup($path,$comment)
   $gpmResult.overallstatus()
   if($error.count -ne 0)
    { 
     funline -strin "An error occurred backing up $(GPMGPO.id)
               it was $($error.categoryInfo)" -scolor red
    } #end if
   else
    {
     $gpmBackup = $gpmResult.result
     write-host "Backed up $($gpmBackup.gpoDisplayName)
               GPO ID: $($gpmBackup.gpoID)
               TimeStamp: $($gpmBackup.timestamp)
               Backup ID: $($gpmBackup.ID)"
    } #end else
  } #end foreach
} #end funbackupgpo
...
if($path) { funBackupGPO }

# SIG # Begin signature block
# MIIkCgYJKoZIhvcNAQcCoIIj+zCCI/cCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUMNoa5LoEPo8rCxXNc2gR+BU2
# sRuggh7hMIIEEjCCAvqgAwIBAgIPAMEAizw8iBHRPvZj7N9AMA0GCSqGSIb3DQEB
...
# AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQU96e+qiT75p0dZy/Q
# aHqraBZ6AnkwXgYKKwYBBAGCNwIBDDFQME6gJoAkAE0AaQBjAHIAbwBzAG8AZgB0
# ACAATABlAGEAcgBuAGkAbgBnoSSAImh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbS9s
# ZWFybmluZyAwDQYJKoZIhvcNAQEBBQAEggEAidGu1K2VjmjVpi0aHNuawKoLAOty
# +pDIQPsIeoVDgbdoMuCy0S4rOyK1Gh4bxIb8s0k/lqnPTwzLT4KaiUdb+Hzy5GKl
# Wsf6mO6UUUAW4h6blC0IPxxoPoI/C++tctOWukQLsFZmL27Q7weTwKF6ObyXfnZA
# u2RgDI276ABa7xnLgaOvwSjCncWPSAw6EMvka+UZrMTkKZLUMfDSGuxqMeK8GELT
# KdEBlR8fhclOlmd/IQuybtkicLsGp/25rk9xCs81M2yuoMO0F7/KkJ0L7B/wdYqe
# 9XtcZG5z4ES3Z4fWK5rAq1UKx2VwRNNP+NZ3P3zBrMEavVlsM1WbsDIqUaGCAh8w
...
# AQkEMRYEFITyIatL3PugXaSIJLAEZDMKooQpMA0GCSqGSIb3DQEBBQUABIIBAB9S
# tGN2NjY8BuMr/9Gti96pKGUo3eNTzGFypLWg14EMY+ALQMgL0H25w7QmL9WiReEQ
# Z6w1EUAs4Q2cTONTd3j+FayfZ1juRKgIdVaRT0ShA+IVBsv3Boyp40Abx8rhf7yf
# svAac8kXpGEj1QedFqdN+oA8z9Xe0SrCL0UKp3O3B3X65a4w2fZD+QUKPyvPKDgC
# E7HrQ69Q2uvP7MBD7AQz75zWCSd1fu/sRQ4nhc6ikr4ztezupTYPK8o6CNFnhPII
# aqq6LPyzWsjmgWQ7tcLB/P64p1RLCZR1Te+YrqmM34yxLTr1coEEItEvymly8sKM
# Icl8zJJ4xS+SRA4n2y4=
