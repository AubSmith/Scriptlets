...
@"
 NAME: QueryBackupLocation.ps1
 DESCRIPTION:
 Queries specified backukp location for the 
...
@"
 ...
 SYNTAX:
 QueryBackupLocation.ps1 

 Displays an error missing parameter, and calls help

 QueryBackupLocation.ps1  -backupFolder c:\gpoBackups

 Lists backups from the c:\gpoBackups folder. Uses default
 sort: Sorts list by time stamp

 QueryBackupLocation.ps1 -backfolder -sort gponame
 
 Lists backups from the c:\gpoBackups folder. Sorts list 
 by gpo name

 QueryBackupLocation.ps1 -backfolder -sort gpodomain
 
 Lists backups from the c:\gpoBackups folder. Sorts list 
 by gpo domain name

 QueryBackupLocation.ps1 -help

 Prints the help topic for the script

 QueryBackupLocation.ps1 -help -full

 Prints full help topic for the script

 QueryBackupLocation.ps1 -help -examples

 Prints only the examples for the script

 QueryBackupLocation.ps1 -examples
...
Function funListBackup()
{
 ...
 if($error.count -ne 0)
   { 
    funline -strIN "An error occurred obtaining a list of backups" -scolor red
    "Please verify that $backupFolder exists"
 ...
 if($error.count -ne 0)
   { 
    funline -strIN "An error occurred querying backups from $backupFolder" -scolor red
    "The error was $($error.categoryInfo)"
    exit
   } #end if
  if($backups.count -eq 0) { "No backups found in $backupFolder" }
  ELSE
   {
    funline("GPO Backups located in the $backupFolder folder")
    switch($sort)
     {
       "gpoName"    { $backups | sort-object -property gpodisplayname }
       "timeStamp"  { $backups | sort-object -property Timestamp }
       "gpoDomain"  { $backups | sort-object -property GPODomain }
       $null        { $backups | sort-object -property Timestamp }
       DEFAULT      { $backups | sort-object -property Timestamp }
     } #end switch  
   } #end else
} #end funListBackup
...
if($examples)  { funhelp }
if($full)      { funhelp }
if($backupFolder)  { funlistBackup }
if(!$backupFolder) { "Missing backup folder location" ; funhelp }

# SIG # Begin signature block
# MIIkCgYJKoZIhvcNAQcCoIIj+zCCI/cCAQExCzAJBgUrDgMCGgUAMGkGCisGAQQB
# gjcCAQSgWzBZMDQGCisGAQQBgjcCAR4wJgIDAQAABBAfzDtgWUsITrck0sYpfvNR
# AgEAAgEAAgEAAgEAAgEAMCEwCQYFKw4DAhoFAAQUixOzDe32DPbNWrxB9wFcghIY
# VEWggh7hMIIEEjCCAvqgAwIBAgIPAMEAizw8iBHRPvZj7N9AMA0GCSqGSIb3DQEB
...
# AgELMQ4wDAYKKwYBBAGCNwIBFTAjBgkqhkiG9w0BCQQxFgQUZJlvoflZMfg1nyYU
# ddySqZh1TLgwXgYKKwYBBAGCNwIBDDFQME6gJoAkAE0AaQBjAHIAbwBzAG8AZgB0
# ACAATABlAGEAcgBuAGkAbgBnoSSAImh0dHA6Ly93d3cubWljcm9zb2Z0LmNvbS9s
# ZWFybmluZyAwDQYJKoZIhvcNAQEBBQAEggEAYChFEeh+/hqnjvm8hB3u4PZEAYjR
# t86QJ/daPo7L0dTzYj5OeUekD5cFpiMZYaAXi3MI6y9lx6FhJ7AUkYJtNqPvrvHO
# VZaapwDn2Xlbabbw8wGvIAucuxhdXV4fD9d7+YVcweYSSSaudvuI9aPniecdGQwJ
# ++qhOvlfXi5V0U1QkD0EBwaYlSmAZ+dED30d6/ZvoFCQZ1e2yrBr+vK0e0Te8NSY
# +rxCWcT5eJT3pCfQdviigtmyL4MQ26mE/BTZsHXBdTo7UJqJVvSb72c3frQWEQJp
# eMbvaWVwOquFxtvlarMmW0yDkUqO8oG/ebrJguJj5q3SnEIUjeQPYDOzxKGCAh8w
...
# hkiG9w0BBwEwHAYJKoZIhvcNAQkFMQ8XDTA4MDEzMTIwNTY1MVowIwYJKoZIhvcN
# AQkEMRYEFI8/OSlwgzzNRLpZqZ0iOG/AvqojMA0GCSqGSIb3DQEBBQUABIIBABYy
# UFinx4w+XJZ/t9nasFrDJxx5kAVVHJ3WSUO8uzXqx1C/eu6kjGaZLi8wHUGXvqmt
# h2/F7opdaAZ1VgJSvNtcq2gY4J2tu+JY+f9J+H9nzrCEdwRiv5UjO3UxXom8j2KC
# bib7XR2bZiptjvfD122HI556UmuqEv6Wpzi4wBe5T2hvvp4g6uH7qdu5VputD4Z2
# ZYxoSJhawaN7RRWL7tVgRQzPsSd1jRy9PUDd+wEiWLVuSivqmSHKogprYPAEZZ5C
# B/rQFYgZWMBcbY8BCuEB1airqN/XEe27tN5Nsj6HJWP2lSaCHaCcbNgJ28y++yqE
# wBuuE20hsoTdR26IHjA=
