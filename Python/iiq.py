# https://docs.github.com/en/enterprise-server@3.8/rest/orgs/members#list-organization-members
# ghe-user-unsuspend %USERNAME%


# Import required Python modules
import configparser, csv, json, logging, requests, ssl, sys
...
if len(sys.argv) != 2:
    print(usage)
    logging.error(
        f"Incorrect usage! Please specify environment when invoking this script. \n {usage}"
    )
    sys.exit(1)
else:
    ...
    while True:
        ...
        try:
            ...
            if option == "1":
                ...
                print("You have selected to run an IIQ extract.")
...
def organization_extract_all():
    ...
    try:
        req = requests.get(
            f"{environment_url}/organizations",
            headers=header,
            verify="cacert")
...
def iam_extract():
    ...
    orgs = [logins_item['login'] for logins_item in logins]
    ...
    for login in orgs:
        try:
            ...
            logging.info(
                f"{report} extract URL = {environment_url}/api/v3/orgs/{login}/{api_url}"
            )
        ...
        try:
            ...
            try:
                ...
                try:
                    ...
                    data = report_url.json
                    
                    if not data:

                        logging.info(f"No {report} in org: {login}.")

                    else:
                        logging.info(f"Writing {report} extract for {login}.")

                        for dicts in data:
                            dicts["organization"] = login
                            dicts["role"] = report
                            
                            headers = dict.keys()

                            # Write to file
                            with open(f'iiq.csv', 'a+', encoding='utf-8') as f:
                                writer = csv.DictWriter(f, fieldnames=headers, restval="MISSING!")
                                writer.writeheader()
                    ...
                    logging.error(
                        f"Unable to write the IIQ extract for {login}",
                        exc_info=True,
                    )
                    sys.exit(1)
            except Exception as e:
